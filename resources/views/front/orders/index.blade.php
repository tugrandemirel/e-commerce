...
@section('content')
    ...
    <div class="page-banner-area page-banner-height-2" data-background="{{ asset('assets/user/img/banner/page-banner-4.jpg') }}">
        <div class="container">
            <div class="row">
                <div class="col-xl-12">
                    <div class="page-banner-content text-center">
                        <h4 class="breadcrumb-title">Geçmiş Siparişlerim</h4>
...
{{--                                    <th class="product-remove">Kaldır</th>--}}
                                ...
                                @foreach($carts as $cart)
                                    <tr>
                                        @if ($cart->product->hasMedia('images'))
                                        <td class="product-thumbnail"><a href="{{ route('front.product.detail', ['slug' => $cart->product->slug]) }}"><img src="{{ $cart->product->getFirstMedia('images')->getUrl() }}" alt=""></a></td>
                                        @else
                                        <td class="product-thumbnail"><a href="{{ route('front.product.detail', ['slug' => $cart->product->slug]) }}"><img src="" alt=""></a></td>
                                        @endif
                                        <td class="product-name"><a href="{{ route('front.product.detail', ['slug' => $cart->product->slug]) }}">{{ $cart->product->title }}</a></td>
                                        <td class="product-name"><a href="#">{{ $cart->product->brand->name }}</a></td>
